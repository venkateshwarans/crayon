...
const ConversationStarterItem = ({
  ...
  if (variant === "short") {
    return (
      <button
        type="button"
        className="crayon-shell-conversation-starter-item-short"
  ...
  return (
    <button
      type="button"
      className="crayon-shell-conversation-starter-item-long"
      onClick={() => onClick(prompt)}
    >
      ...
      {starters.map((item, index) => (
        <Fragment key={`${item.displayText}-${index}`}>
          ...
          {variant === "long" && index < starters.length - 1 && (
            <div className="crayon-shell-conversation-starter__separator">
              <Separator />
            </div>
          )}
        </Fragment>
